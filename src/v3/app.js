...
function empty(title,text,action){ return el('div',{class:'empty'},el('div',{class:'empty-mark','aria-hidden':'true'},el('span',{text:'+'})),el('h3',{text:title}),el('p',{text}),action||null); }
...
function firstProfile(){
  ...
  const intro=el('div',{class:'onboarding-intro'},el('div',{class:'onboarding-mark','aria-hidden':'true'},'M'),el('div',{},el('p',{class:'eyebrow',text:'\u0412\u0421\u0422\u0420\u0415\u0427\u0410\u0415\u0422 \u041f\u0410\u0412\u0415\u041b \u041c\u0410\u0420\u041a\u041e\u0412'}),el('h2',{text:'\u0412\u0441\u044f \u0438\u0441\u0442\u043e\u0440\u0438\u044f \u0437\u0434\u043e\u0440\u043e\u0432\u044c\u044f \u2014 \u0432 \u043e\u0434\u043d\u043e\u0439 \u0441\u0438\u0441\u0442\u0435\u043c\u0435'}),el('p',{text:'\u042f \u0441\u043e\u0431\u0440\u0430\u043b Health OS, \u0447\u0442\u043e\u0431\u044b \u043c\u0435\u0434\u0438\u0446\u0438\u043d\u0441\u043a\u0438\u0435 \u0434\u0430\u043d\u043d\u044b\u0435 \u0431\u044b\u043b\u0438 \u0432 \u043e\u0434\u043d\u043e\u043c \u043c\u0435\u0441\u0442\u0435 \u2014 \u0441 \u043a\u043e\u043d\u0442\u0435\u043a\u0441\u0442\u043e\u043c \u0438 \u0434\u0438\u043d\u0430\u043c\u0438\u043a\u043e\u0439.'})));
  const steps=el('div',{class:'onboarding-grid'},el('div',{class:'onboarding-step'},el('span',{text:'01'}),el('strong',{text:'\u0421\u043e\u0437\u0434\u0430\u0439\u0442\u0435 \u043f\u0440\u043e\u0444\u0438\u043b\u044c'}),el('small',{text:'\u041c\u0438\u043d\u0438\u043c\u0443\u043c \u0434\u0430\u043d\u043d\u044b\u0445, \u0442\u043e\u043b\u044c\u043a\u043e \u043d\u0430 \u0432\u0430\u0448\u0435\u043c \u0443\u0441\u0442\u0440\u043e\u0439\u0441\u0442\u0432\u0435.'})),el('div',{class:'onboarding-step'},el('span',{text:'02'}),el('strong',{text:'\u0414\u043e\u0431\u0430\u0432\u044c\u0442\u0435 \u043e\u0434\u043d\u0443 \u0442\u043e\u0447\u043a\u0443'}),el('small',{text:'\u041f\u043e\u0441\u043b\u0435\u0434\u043d\u0438\u0439 \u0430\u043d\u0430\u043b\u0438\u0437, \u0432\u0435\u0441 \u0438\u043b\u0438 \u0441\u043e\u043d \u2014 \u043d\u0430 \u0432\u044b\u0431\u043e\u0440.'})),el('div',{class:'onboarding-step'},el('span',{text:'03'}),el('strong',{text:'\u0421\u043c\u043e\u0442\u0440\u0438\u0442\u0435 \u0434\u0438\u043d\u0430\u043c\u0438\u043a\u0443'}),el('small',{text:'\u041e\u0434\u0438\u043d \u043f\u043e\u043a\u0430\u0437\u0430\u0442\u0435\u043b\u044c \u043f\u043e\u0447\u0442\u0438 \u043d\u0438\u043a\u043e\u0433\u0434\u0430 \u043d\u0435 \u0440\u0430\u0441\u0441\u043a\u0430\u0437\u044b\u0432\u0430\u0435\u0442 \u0432\u0441\u044e \u0438\u0441\u0442\u043e\u0440\u0438\u044e.'})));
  wrap.prepend(intro,steps);
  const demo=button('\u0421\u043d\u0430\u0447\u0430\u043b\u0430 \u043e\u0442\u043a\u0440\u044b\u0442\u044c \u0441\u0438\u043d\u0442\u0435\u0442\u0438\u0447\u0435\u0441\u043a\u0438\u0439 demo',()=>{closeDialog();createDemoProfile();},'secondary');
  dialog('\u0414\u043e\u0431\u0440\u043e \u043f\u043e\u0436\u0430\u043b\u043e\u0432\u0430\u0442\u044c \u0432 Markov Health OS',wrap,[demo]); $('#dialog').addEventListener('cancel',e=>e.preventDefault(),{once:true});
}
...
function render(){
  if(!S.data)return; applyTheme(); const hash=location.hash.slice(1),[route,queryString]=hash.split('?'); S.route=route||S.route||'dashboard'; const profile=activeProfile();
  $('#profile-name').textContent=profile?.name||'Health OS'; $('#app-version').textContent=`v${APP_VERSION} / schema ${SCHEMA_VERSION}`;
  $$('.nav-link').forEach(a=>{const active=a.dataset.route===S.route;a.classList.toggle('active',active);if(active)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});
  ...
  if(S.route==='labs'&&S.selectedAnalyte){const analyte=findAnalyte(S.selectedAnalyte);const rows=records('labResults').filter(x=>(x.analyteId||normalizeText(x.canonicalName||x.originalName))===(analyte?.id||normalizeText(S.selectedAnalyte)));if(analyte&&rows.length)main.append(renderAnalyteEducation(analyte,rows));}
  if(S.route==='ai')main.append(aiScopePanel());
  if(S.route==='brief')main.append(authorNote('Заметка Павла Маркова','Doctor Brief — это способ сделать разговор с врачом содержательнее. Он не заменяет консультацию и не выбирает за вас медицинские решения.','compact'));
  if(queryString&&new URLSearchParams(queryString).get('action')==='quick-add'){history.replaceState(null,'','#dashboard');setTimeout(openQuickAdd,0);}
}
function aiScopePanel(){return el('section',{class:'panel ai-scope'},el('div',{class:'section-head'},el('div',{},el('h2',{text:'Границы AI-разбора'}),el('p',{text:'Прозрачность важнее впечатляющего ответа.'}))),el('div',{class:'scope-grid'},el('div',{class:'scope-card included'},el('span',{text:'AI получит'}),el('strong',{text:'Только показанный JSON-контекст'}),el('small',{text:'Факты и расчёты подготовлены кодом; вы увидите их до отправки.'})),el('div',{class:'scope-card excluded'},el('span',{text:'AI не получит'}),el('strong',{text:'Данные других профилей и исходные файлы'}),el('small',{text:'Также не отправляются скрытые данные, ключи и содержимое, которого нет в preview.'})),el('div',{class:'scope-card caution'},el('span',{text:'AI не предназначен для'}),el('strong',{text:'Диагноза, назначения лечения или экстренной помощи'}),el('small',{text:'Используйте результат как навигацию и список вопросов для врача.'}))));}
function renderAnalyteEducation(analyte,rows){
  const related=records('labResults').filter(x=>x.analyteId&&x.analyteId!==analyte.id&&findAnalyte(x.analyteId)?.category===analyte.category).sort((a,b)=>String(b.collectedAt||'').localeCompare(String(a.collectedAt||''))).slice(0,4);
  const questions=['Как этот результат сопоставляется с предыдущими измерениями?','Нужно ли учитывать условия сдачи, время или лекарства?','Есть ли смысл повторить показатель или посмотреть связанные анализы?'];
  const section=(title,body)=>el('article',{class:'education-card'},el('h3',{text:title}),el('p',{text:body}));
  const panel=el('section',{class:'panel education-panel'},el('div',{class:'section-head'},el('div',{},el('h2',{text:'Как читать показатель'}),el('p',{text:'Справочная навигация, а не индивидуальное медицинское заключение.'})),el('span',{class:'pill',text:analyte.category||'Лаборатория'})));
  panel.append(el('div',{class:'education-grid'},section('Что это',analyte.descriptionRu||'Краткое описание для этого показателя ещё не добавлено.'),section('Зачем измеряют','Результат используют как одну часть общей картины. Его смысл зависит от метода, единиц, референса этой лаборатории и клинического контекста.'),section('Что может влиять','На результат и его сопоставимость могут влиять время сдачи, подготовка, самочувствие, лекарства, добавки, нагрузка и особенности метода.'),section('Как читать результат','Начните с исходных единиц и референсного диапазона в отчёте. Затем сравните с предыдущими точками в той же единице и отметьте контекст. Не подменяйте лабораторный референс универсальной «идеальной нормой».')));
  panel.append(el('div',{class:'education-split'},el('div',{},el('h3',{text:'Что обсудить с врачом'}),el('ul',{},...questions.map(x=>el('li',{text:x})))),el('div',{},el('h3',{text:'Связанные показатели'}),related.length?el('div',{class:'related-list'},...related.map(x=>button(`${x.canonicalName} · ${formatTrendValue(x)}`,()=>{S.selectedAnalyte=x.analyteId;render();},'link-button'))):el('p',{class:'muted',text:'В сохранённых данных пока нет других показателей этой группы.'}))));
  if(analyte.sourceReferences?.length)panel.append(el('details',{class:'source-details'},el('summary',{text:'Методологическая заметка и источники каталога'}),el('p',{text:analyte.sourceReferences.join(' · ')})));
  return panel;
}
...
function authorNote(title,text,kind='note'){ return el('aside',{class:`author-note ${kind}`},el('span',{class:'author-avatar','aria-hidden':'true'},'PM'),el('div',{},el('strong',{text:title}),el('p',{text}))); }
function formatTrendValue(result){ return `${fmtNum(result.canonicalValue)} ${result.canonicalUnit||''}`.trim(); }
function trendPanel(summary){
  const rows=[];
  for(const current of summary.current){
    const history=records('labResults').filter(x=>(x.analyteId||normalizeText(x.canonicalName||x.originalName))===(current.analyteId||normalizeText(current.canonicalName||current.originalName))).filter(x=>Number.isFinite(Number(x.canonicalValue))).sort((a,b)=>String(a.collectedAt||'').localeCompare(String(b.collectedAt||'')));
    if(history.length>=2) rows.push({current,history});
  }
  const panel=el('section',{class:'panel trend-panel'},el('div',{class:'section-head'},el('div',{},el('h2',{text:'Динамика показателей'}),el('p',{text:'Только повторные измерения в сопоставимых единицах.'})),button('Все анализы',()=>{location.hash='labs';},'secondary')));
  if(!rows.length){panel.append(empty('Динамика появится здесь','Добавьте второй результат одного показателя — Health OS покажет направление и даты, не придумывая индивидуальную цель.',button('Добавить анализ',openLabImport,'primary')));return panel;}
  const list=el('div',{class:'trend-list'});
  for(const row of rows.slice(0,4)){
    const values=row.history,latest=values.at(-1),first=values[0],change=delta(latest.canonicalValue,first.canonicalValue);
    const title=el('button',{type:'button',class:'trend-title',text:latest.canonicalName,onclick:()=>{S.selectedAnalyte=latest.analyteId||latest.canonicalName;location.hash='labs';render();}});
    list.append(el('article',{class:'trend-row'},el('div',{},title,el('small',{text:`${formatTrendValue(first)} → ${formatTrendValue(latest)}`})),sparkline(values),el('div',{class:'trend-meta'},el('strong',{text:change?.percent==null?'—':`${change.percent>0?'+':''}${fmtNum(change.percent,1)}%`}),el('small',{text:`${values.length} ${values.length===1?'точка':'точки'}`}))));
  }
  panel.append(list,el('p',{class:'chart-note',text:'Динамика помогает заметить изменение, но не объясняет его причину и не заменяет клиническую интерпретацию.'}));
  return panel;
}
...
function renderDashboard(root){
  ...
  root.append(el('section',{class:'dashboard-intro'},el('div',{},el('p',{class:'eyebrow',text:'СЕГОДНЯ · '+fmtDate(new Date())}),el('h2',{text:`Здравствуйте, ${profile?.name||'это ваш Health OS'}.`}),el('p',{text:'Сигналы, динамика и контекст — в одном спокойном рабочем пространстве.'})),authorNote('Павел Марков','Один показатель почти никогда не рассказывает всю историю. Смотрите на динамику, лабораторный референс, условия сдачи и общий контекст.','compact')));
  ...
  root.append(trendPanel(summary));
...
function sparkline(rows){
  const values=rows.filter(r=>Number.isFinite(Number(r.canonicalValue))),vals=values.map(r=>Number(r.canonicalValue));
  const summary=values.map(r=>`${fmtDate(r.collectedAt)}: ${formatTrendValue(r)}`).join('; ');
  const box=el('div',{class:'spark',role:'img','aria-label':`История показателя. ${summary}`});
  if(values.length<2){box.append(el('p',{class:'muted',text:'Нужно минимум две точки, чтобы показать динамику.'}));return box;}
  const min=Math.min(...vals),max=Math.max(...vals),span=max-min||1,pad=14,w=600,h=180;
  const svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.setAttribute('viewBox',`0 0 ${w} ${h}`);svg.setAttribute('aria-hidden','true');
  const grid=(y)=>{const line=document.createElementNS('http://www.w3.org/2000/svg','line');line.setAttribute('x1',pad);line.setAttribute('x2',w-pad);line.setAttribute('y1',y);line.setAttribute('y2',y);line.setAttribute('class','chart-gridline');svg.append(line);};
  grid(28);grid(90);grid(152);
  const lo=Number(values[0].referenceLow),hi=Number(values[0].referenceHigh);
  if(Number.isFinite(lo)||Number.isFinite(hi)){
    const bandLow=Number.isFinite(lo)?Math.max(0,Math.min(1,(lo-min)/span)):0,bandHigh=Number.isFinite(hi)?Math.max(0,Math.min(1,(hi-min)/span)):1;
    const band=document.createElementNS('http://www.w3.org/2000/svg','rect');band.setAttribute('x',pad);band.setAttribute('width',w-pad*2);band.setAttribute('y',h-pad-(bandHigh*(h-pad*2)));band.setAttribute('height',Math.max(2,(bandHigh-bandLow)*(h-pad*2)));band.setAttribute('class','reference-band');svg.append(band);
  }
  const points=values.map((r,i)=>{const x=pad+(i/(values.length-1))*(w-pad*2),y=h-pad-((Number(r.canonicalValue)-min)/span)*(h-pad*2);return `${x},${y}`;});
  const poly=document.createElementNS('http://www.w3.org/2000/svg','polyline');poly.setAttribute('points',points.join(' '));poly.setAttribute('class','chart-line');svg.append(poly);
  values.forEach((r,i)=>{const [x,y]=points[i].split(',');const circle=document.createElementNS('http://www.w3.org/2000/svg','circle');circle.setAttribute('cx',x);circle.setAttribute('cy',y);circle.setAttribute('r','5');circle.setAttribute('class','chart-point');circle.setAttribute('tabindex','0');circle.setAttribute('aria-label',`${fmtDate(r.collectedAt)} — ${formatTrendValue(r)}`);svg.append(circle);});
  box.append(svg,el('div',{class:'spark-labels'},el('span',{text:fmtDate(values[0].collectedAt)}),el('span',{text:fmtDate(values.at(-1).collectedAt)})));return box;
}
...
function renderTimeline(root){
  ...
  root.append(pageHead(RU.timeline,'Единая история измерений, отчётов и контекста.',[button('+ '+RU.add,openQuickAdd,'primary')]));
  const filter=input('q','search','',{placeholder:'Поиск по истории'}),typeFilter=select('timeline-type',[['','Все типы'],...new Map(items.map(x=>[x.type,x.type])).entries()]);
  ...
  const draw=()=>{
    ...
    for(const x of items){
      ...
      if(typeFilter.value&&x.type!==typeFilter.value)continue;
  ...
  filter.addEventListener('input',draw);typeFilter.addEventListener('change',draw);
  root.append(el('section',{class:'panel'},el('div',{class:'timeline-toolbar'},field('Поиск',filter),field('Тип записи',typeFilter)),list));
  draw();
}
...
function renderAI(root){ const p=activeProfile(),config=S.data.settings.ai||{}; const context=buildAIContext(S.data,p.id,'overview'); root.append(pageHead(RU.ai,'\u041a\u043e\u0434 \u0441\u0447\u0438\u0442\u0430\u0435\u0442. AI \u043e\u0431\u044a\u044f\u0441\u043d\u044f\u0435\u0442. \u041d\u0438\u0447\u0435\u0433\u043e \u043d\u0435 \u043e\u0442\u043f\u0440\u0430\u0432\u043b\u044f\u0435\u0442\u0441\u044f \u0431\u0435\u0437 \u0432\u0430\u0448\u0435\u0433\u043e \u0434\u0435\u0439\u0441\u0442\u0432\u0438\u044f.')); const purpose=select('purpose',[['overview','\u0420\u0430\u0437\u043e\u0431\u0440\u0430\u0442\u044c \u0434\u0430\u043d\u043d\u044b\u0435'],['changes','\u0427\u0442\u043e \u0438\u0437\u043c\u0435\u043d\u0438\u043b\u043e\u0441\u044c'],['analyte','\u041e\u0431\u044a\u044f\u0441\u043d\u0438\u0442\u044c \u043f\u043e\u043a\u0430\u0437\u0430\u0442\u0435\u043b\u044c'],['doctor','\u0412\u043e\u043f\u0440\u043e\u0441\u044b \u0432\u0440\u0430\u0447\u0443']]),consent=input('consent','checkbox',''),preview=el('pre',{class:'json-preview',text:JSON.stringify(context,null,2)}),result=el('div',{class:'ai-result'}); const send=button('\u041e\u0442\u043f\u0440\u0430\u0432\u0438\u0442\u044c \u043d\u0430 AI-\u0440\u0430\u0437\u0431\u043e\u0440',async()=>{if(!consent.checked){toast('\u041d\u0443\u0436\u043d\u043e \u044f\u0432\u043d\u043e\u0435 \u0441\u043e\u0433\u043b\u0430\u0441\u0438\u0435');return;} const gw=new AIGateway(config); if(!gw.configured()){toast('AI gateway не настроен. Сначала укажите same-origin endpoint в настройках.');return;} send.disabled=true; result.textContent='\u041e\u0436\u0438\u0434\u0430\u043d\u0438\u0435 \u043e\u0442\u0432\u0435\u0442\u0430...'; try{const c=buildAIContext(S.data,p.id,purpose.value),r=await gw.request(purpose.value,c);renderAIResult(result,r);}catch(e){result.textContent=`\u041d\u0435 \u0443\u0434\u0430\u043b\u043e\u0441\u044c \u043f\u043e\u043b\u0443\u0447\u0438\u0442\u044c \u0431\u0435\u0437\u043e\u043f\u0430\u0441\u043d\u044b\u0439 AI-\u043e\u0442\u0432\u0435\u0442: ${e.message}`;}finally{send.disabled=false;}},'primary'); root.append(el('section',{class:'panel stack'},field('\u0426\u0435\u043b\u044c',purpose),el('div',{class:'privacy-box'},el('strong',{text:'\u041f\u0440\u0435\u0432\u044c\u044e \u043f\u0435\u0440\u0435\u0434\u0430\u0432\u0430\u0435\u043c\u043e\u0433\u043e \u043a\u043e\u043d\u0442\u0435\u043a\u0441\u0442\u0430'}),el('p',{text:config.endpoint?`Gateway: ${config.providerLabel||config.endpoint}`:'AI gateway \u043d\u0435 \u043d\u0430\u0441\u0442\u0440\u043e\u0435\u043d. \u041e\u0441\u043d\u043e\u0432\u043d\u043e\u0439 Health OS \u0440\u0430\u0431\u043e\u0442\u0430\u0435\u0442 \u0431\u0435\u0437 AI.'}),preview),el('label',{class:'check'},consent,el('span',{text:'\u042f \u043f\u043e\u0434\u0442\u0432\u0435\u0440\u0436\u0434\u0430\u044e \u043f\u0435\u0440\u0435\u0434\u0430\u0447\u0443 \u043f\u043e\u043a\u0430\u0437\u0430\u043d\u043d\u043e\u0433\u043e \u0432\u044b\u0448\u0435 \u043a\u043e\u043d\u0442\u0435\u043a\u0441\u0442\u0430 \u0432\u043d\u0435\u0448\u043d\u0435\u043c\u0443 AI-\u0441\u0435\u0440\u0432\u0438\u0441\u0443.'})),send,result)); }
...
function renderSettings(root){
  ...
  dataPanel.append(
    ...
    el('div',{class:'action-row'},
      button('Экспорт JSON',async()=>{const payload=await S.repo.exportBackupWithChecksum();download(`markov-health-os-${new Date().toISOString().slice(0,10)}.json`,JSON.stringify(payload,null,2),'application/json');}),
      button('Экспорт .mhos',exportEncryptedBackup),
...
async function exportEncryptedBackup(){
  ...
  }catch(e){toast(`Не удалось зашифровать backup: ${e.message}`);}
}
function openBackupImport(){
  ...
  f.addEventListener('change',async()=>{
    try{
      ...
      S.data=S.repo.importBackup(payload);closeDialog();render();toast('Backup импортирован');
    }catch(e){toast(`Не удалось импортировать backup: ${e.message}`);}
  });
  dialog('Import backup',wrap,[button(RU.close,closeDialog)]);
}

function openQuickAdd(){
  const choices=[['weight','Масса','Тело'],['bp','Давление','Тело'],['body','Пульс, HRV и другое','Тело'],['sleep','Сон','Восстановление'],['training','Тренировка','Восстановление'],['symptom','Симптом','Контекст'],['event','Событие','Контекст'],['medication','Лекарство','Контекст'],['supplement','Добавка','Контекст'],['nutrition','Питание','Контекст'],['note','Заметка','Контекст'],['goal','Цель','Планирование'],['labs','Анализы','Данные']];
  const query=input('quick-search','search','',{placeholder:'Найти действие…',autocomplete:'off'}),list=el('div',{class:'quick-groups'}),recent=choices.slice(0,3);
  const action=(key)=>{closeDialog();key==='labs'?openLabImport():openRecordForm(key);};
  const draw=()=>{const q=normalizeText(query.value);list.replaceChildren();const grouped=new Map();for(const row of choices.filter(x=>!q||normalizeText(`${x[1]} ${x[2]}`).includes(q))){if(!grouped.has(row[2]))grouped.set(row[2],[]);grouped.get(row[2]).push(row);}if(!q){const recentBox=el('section',{class:'quick-group'},el('h3',{text:'Часто используемое'}),el('div',{class:'quick-grid'},...recent.map(([k,label])=>button(label,()=>action(k),'quick'))));list.append(recentBox);}for(const [group,rows] of grouped){list.append(el('section',{class:'quick-group'},el('h3',{text:group}),el('div',{class:'quick-grid'},...rows.map(([k,label])=>button(label,()=>action(k),'quick')))));}if(!list.children.length)list.append(empty('Ничего не найдено','Попробуйте «сон», «анализы» или «заметка».'));};query.addEventListener('input',draw);draw();dialog('Быстро добавить',el('div',{class:'stack'},el('p',{class:'muted',text:'Одно действие — одна запись. Дата и профиль подставлены автоматически.'}),query,list),[button(RU.close,closeDialog)]);query.focus();
}
...
function importStepper(active='manual'){const labels=[['manual','1 · Загрузили'],['review','2 · Распознали'],['confirm','3 · Проверяем'],['saved','4 · Сохраняем']];return el('ol',{class:'import-stepper','aria-label':'Шаги импорта'},...labels.map(([key,label],i)=>el('li',{class:key===active?'active':''},el('span',{text:String(i+1)}),el('strong',{text:label}))));}
function openLabImport(){ S.importCandidates=[];S.importMeta={profileId:activeProfile().id,sourceType:'manual',collectedAt:new Date().toISOString().slice(0,10),laboratoryName:'',sourceFileName:null}; const tabs=el('div',{class:'tab-row'}),body=el('div',{class:'stack'}),steps=importStepper(); const modes=[['manual','\u0412\u0440\u0443\u0447\u043d\u0443\u044e'],['csv','CSV'],['pdf','PDF'],['image','\u0424\u043e\u0442\u043e / OCR']]; const choose=m=>{body.replaceChildren(); $$('.tab-row button',tabs).forEach(x=>x.classList.toggle('active',x.dataset.mode===m)); if(m==='manual')manualImport(body); if(m==='csv')fileImport(body,'csv'); if(m==='pdf')fileImport(body,'pdf'); if(m==='image')imageImport(body);}; for(const [m,label] of modes)tabs.append(el('button',{type:'button','data-mode':m,class:'tab',text:label,onclick:()=>choose(m)})); const wrap=el('div',{class:'stack'},steps,tabs,body); dialog(RU.addLabs,wrap,[button(RU.cancel,closeDialog)]); choose('manual'); }
...
function fileImport(root,kind){ const file=input('file','file','',{accept:kind==='csv'?'.csv,.tsv,text/csv,text/tab-separated-values':'.pdf,application/pdf'}),status=el('p',{class:'muted',role:'status','aria-live':'polite'}),rows=el('div',{class:'candidate-list'}); root.append(importMetaFields(),field(kind.toUpperCase(),file),status,rows,button('\u041f\u0440\u043e\u0432\u0435\u0440\u0438\u0442\u044c \u0438 \u0441\u043e\u0445\u0440\u0430\u043d\u0438\u0442\u044c',()=>reviewCandidates(),'primary')); file.addEventListener('change',async()=>{const f=file.files[0],check=validateUpload(f,kind); if(!check.ok){status.textContent=check.error;return;} S.importMeta.sourceType=kind;S.importMeta.sourceFileName=f.name; try{if(kind==='csv'){const parsed=parseCsv(await f.text());S.importCandidates=parsed.rows;status.textContent=`Найдено строк: ${parsed.rows.length}; ${parsed.errors.join(', ')||'ошибок не найдено'}`;}else{const ext=extractPdfTextLight(await f.arrayBuffer());S.importCandidates=parseLabText(ext.text);status.textContent=`Найдено кандидатов: ${S.importCandidates.length}. PDF: ${ext.limitations}.`; if(!S.importCandidates.length)status.textContent+=' Сканированный или сжатый PDF может потребовать OCR.';}renderCandidateList(rows);}catch(e){status.textContent=`Import error: ${e.message}`;}}); }
function imageImport(root){
  ...
  const rows=el('div',{class:'candidate-list'}),status=el('p',{class:'muted',role:'status','aria-live':'polite'}),consent=input('ocrConsent','checkbox','');
  ...
  const parse=()=>{S.importMeta.sourceType='ocr';S.importMeta.sourceFileName=file.files[0]?.name||null;S.importCandidates=parseLabText(text.value);renderCandidateList(rows);status.textContent=`Найдено кандидатов: ${S.importCandidates.length}`;};
  const run=button('\u0420\u0430\u0441\u043f\u043e\u0437\u043d\u0430\u0442\u044c \u0447\u0435\u0440\u0435\u0437 OCR gateway',async()=>{
    ...
    if(!consent.checked){status.textContent='Подтвердите согласие на передачу файла.';return;}
    const gateway=new OCRGateway(config); if(!gateway.configured()){status.textContent='OCR gateway не настроен. Файл не отправлялся.';return;}
    ...
    catch(e){status.textContent=`Не удалось распознать файл: ${e.message}`;} finally{run.disabled=false;}
  });
...
function openSearch(){
  S.searchOpen=true;
  const d=$('#search-dialog'),q=$('#global-search'),out=$('#search-results');let activeIndex=0,items=[];
  const nav=[['dashboard','Обзор','Состояние и свежесть данных'],['labs','Анализы','Отчёты, показатели и динамика'],['timeline','История','Единая временная линия'],['brief','Врачу','Подготовить консультацию'],['ai','AI-разбор','Объяснить выбранный контекст'],['settings','Настройки','Профили, privacy и backup']];
  const commands=[['+ Масса','Добавить измерение',()=>openRecordForm('weight')],['+ Давление','Добавить измерение',()=>openRecordForm('bp')],['+ Сон','Добавить восстановление',()=>openRecordForm('sleep')],['+ Событие','Добавить контекст',()=>openRecordForm('event')],['+ Анализы','Импортировать отчёт',openLabImport],['+ Doctor Brief','Создать сводку для врача',()=>{location.hash='brief';closeSearch();}],['+ Backup','Экспортировать backup',()=>{location.hash='settings';closeSearch();}]];
  const entities=()=>[
    ...records('labResults').map(x=>({label:`Показатель · ${x.canonicalName||x.originalName}`,detail:formatTrendValue(x),run:()=>{S.selectedAnalyte=x.analyteId||x.canonicalName;location.hash='labs';closeSearch();render();}})),
    ...records('medications').map(x=>({label:`Лекарство · ${x.name}`,detail:x.dose||'',run:()=>{location.hash='timeline';closeSearch();}})),
    ...records('supplements').map(x=>({label:`Добавка · ${x.name}`,detail:x.dose||'',run:()=>{location.hash='timeline';closeSearch();}})),
    ...records('notes').map(x=>({label:`Заметка · ${x.title||'Без заголовка'}`,detail:x.text||'',run:()=>{location.hash='timeline';closeSearch();}}))
  ];
  const draw=()=>{
    const needle=normalizeText(q.value),found=[];
    for(const [route,label,detail] of nav)if(!needle||normalizeText(`${label} ${detail}`).includes(needle))found.push({label,detail,category:'Раздел',run:()=>{location.hash=route;closeSearch();}});
    for(const [label,detail,run] of commands)if(!needle||normalizeText(`${label} ${detail}`).includes(needle))found.push({label,detail,category:'Действие',run:()=>{closeSearch();run();}});
    for(const analyte of searchAnalytes(q.value,12))found.push({label:analyte.canonicalRu,detail:`${analyte.shortRu||analyte.englishName||''} · ${analyte.category||'Показатель'}`,category:'Каталог',run:()=>{S.selectedAnalyte=analyte.id;location.hash='labs';closeSearch();render();}});
    for(const entity of entities().filter(x=>!needle||normalizeText(`${x.label} ${x.detail}`).includes(needle)).slice(0,10))found.push({...entity,category:'Мои данные'});
    items=found.slice(0,24);activeIndex=Math.min(activeIndex,Math.max(0,items.length-1));out.replaceChildren(...items.map((x,i)=>el('button',{type:'button',class:`search-hit${i===activeIndex?' active':''}`,role:'option','aria-selected':i===activeIndex?'true':'false',onclick:()=>x.run()},el('span',{class:'search-hit-main'},el('strong',{text:x.label}),el('small',{text:x.detail})),el('span',{class:'search-hit-category',text:x.category}))));
  };
  q.value='';q.oninput=()=>{activeIndex=0;draw();};q.onkeydown=e=>{if(e.key==='ArrowDown'){e.preventDefault();activeIndex=Math.min(activeIndex+1,items.length-1);draw();}if(e.key==='ArrowUp'){e.preventDefault();activeIndex=Math.max(activeIndex-1,0);draw();}if(e.key==='Enter'&&items[activeIndex]){e.preventDefault();items[activeIndex].run();}};draw();d.showModal();q.focus();
}
